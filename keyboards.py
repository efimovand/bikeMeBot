...
class NoBalanceCallback(CallbackData, prefix="no_balance"):
    action: str  # "topup" | "cancel"
...
def no_balance_keyboard() -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(
        text="⭐️ Пополнить баланс",
        callback_data=NoBalanceCallback(action="topup"),
    )
    builder.button(
        text="◀️ Назад",
        callback_data=NoBalanceCallback(action="cancel"),
    )
    builder.adjust(1)
    return builder.as_markup()
