...
from states import GloveStates, OnboardingStates
...
@router.callback_query(BackCallback.filter((F.entity == "glove") & (F.step == "to_brand")), GloveStates.choosing_model)
async def on_glove_back_to_brand(query: CallbackQuery, state: FSMContext):
    await query.answer()
...
@router.callback_query(BackCallback.filter((F.entity == "glove") & (F.step == "to_model")), GloveStates.choosing_color)
async def on_glove_back_to_model(query: CallbackQuery, state: FSMContext):
    await query.answer()
    data = await state.get_data()
    brand = data.get("brand", "")
...
@router.callback_query(BackCallback.filter((F.entity == "glove") & (F.step == "to_menu")), GloveStates.choosing_brand)
async def on_glove_cancel(query: CallbackQuery, state: FSMContext):
    await query.answer()
    data = await state.get_data()
    onboarding = data.get("onboarding", False)
    if onboarding:
