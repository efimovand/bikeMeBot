...
async def generate_for_user(
    ...
    # Image order matters for face retention. Empirically, putting the bike
    # FIRST (as the base scene) and the person photos AFTER preserves facial
    # identity and body proportions better than the person-first ordering.
    # Images: 1 = bike, 2-4 = person, 5+ = gear.
    # NOTE: must stay in sync with build_image_index() in prompts.py.
    items: list[tuple[str | Path, str | None]] = [
        (bike_file_path, None),                                              # Image 1
        (BASE_DIR / user.photoset.front_photo, f"{tg_id}_front_{ts}.jpg"),   # Image 2
        (BASE_DIR / user.photoset.side_photo, f"{tg_id}_side_{ts}.jpg"),     # Image 3
        (BASE_DIR / user.photoset.body_photo, f"{tg_id}_body_{ts}.jpg"),     # Image 4
    ]
    if helmet_file_path:
